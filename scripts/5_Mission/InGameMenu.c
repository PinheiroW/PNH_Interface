modded class InGameMenu extends UIScriptedMenu
{
	...
	protected ref TextWidget 	m_DevText;
	protected ref ImageWidget 	m_LogoServer;
	protected Widget 			m_DiscordButton;
	protected Widget 			m_SiteButton;

	override Widget Init()
	{
		layoutRoot = GetGame().GetWorkspace().CreateWidgets("PNH_Interface/data/layouts/day_z_ingamemenu.layout");
		...
		m_SiteButton				= layoutRoot.FindAnyWidget("sitebtn");
	...
	void SetInterface()
	{
		m_SeparatorPanel.SetColor(COLOR_HUD);
		m_DevText.SetText(TEXT_CREDITS);
		m_LogoServer.LoadImageFile( 0, IMAGE_LOGO );
	}	
