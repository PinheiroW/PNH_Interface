modded class CharacterCreationMenu extends UIScriptedMenu
{
	...
	protected Widget m_character_header;
	protected Widget m_menu_details_tooltip_header;
	...
	override Widget Init()
	{
		...
		m_character_header					= layoutRoot.FindAnyWidget("character_header");
		m_menu_details_tooltip_header		= layoutRoot.FindAnyWidget("menu_details_tooltip_header");
		...
		m_character_header.SetColor(COLOR_HUD);
		m_menu_details_tooltip_header.SetColor(COLOR_HUD);
	...
	override void ColorHighlight(Widget w)
	{
		...
		if (text1)
		{
			text1.SetColor(COLOR_TEXT);
		}
		
		if (text2)
		{
			text2.SetColor(COLOR_TEXT);
		}
		
		if (text3)
		{
			text3.SetColor(COLOR_TEXT);
			w.SetAlpha(1);
		}
		...
		if (option)
		{
			option.SetColor(COLOR_TEXT);
		}
		
		#ifndef PLATFORM_CONSOLE
		if (option_label)
		{
			option_label.SetColor(COLOR_TEXT);
		}
		#endif
	}	
}